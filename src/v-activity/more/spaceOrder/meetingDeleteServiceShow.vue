<template>
  <div class="select-service" @click.stop="closeOwn">
    ...
    <div v-if="callServiceShow && entity.type !== 'MANAGER'">
...
<script>
  ...
  export default {
    data() {
      return {
        prompt:"在两小时前取消订单可全额退款，两小时后只能删除不能退款，谢谢！",
    ...
    methods: {
      ...
      successCallback(){
        this.$store.state.plus.push.createMessage("您已申请会议室退款。",undefined,{prompt:"会议室退款申请已提交"});
      },
      closeAllPopup(){
      },
      selectYes(e) {
        let that = this;
        let base = new Base();
        base.httpPost("/space/reservation/delete/meeting/pay/", { uuid:e.uuid }, function (res){
