<template>
  <div class="order-invite-container">
    <div class="order-top">
      <div class="close-ico">
    ...
    <div class="order-list-item">
      ...
      >
        <div class="order-list-item-container">
          <div class="list-item " v-for="(inviteVisitor,index) in pager.data" data-type="0" :key="index">
            <div class="list-box" @touchstart.capture="touchStart" @touchend.capture="touchEnd" @click="skip">
              <div class="order-item-top">
                <div class="order-item-top-title">
                  名称：{{inviteVisitor.name}}
                </div>
                <div class="order-item-top-edit" @click.stop="orderEdit(inviteVisitor)">编辑</div>
              </div>
              <div class="remarks">
              ...
              <div class="room-item-footer">
                <div class="room-order-footer-time">
                  {{inviteVisitor.invitingTime | allDay }}/{{inviteVisitor.accessTime | date("HH:ss") }}
                </div>
                <div class="room-order-footer-day">
                  {{inviteVisitor.createTime | allDay }}
                </div>
              </div>
            </div>
            <div class="delete" @click="openCallService(inviteVisitor)" :data-index="inviteVisitor.index">取消</div>
          </div>
        </div>
      </scroller>

      <div class="popup-box" v-if="callServiceShow">
        <CancelActive :title="title" :closeCall="closeAllPopup" :entity="inviteVisitor" :refresh="refresh"></CancelActive>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default{
    ...
    methods : {
      ...
      openCallService(inviteVisitor) {
        this.inviteVisitor = inviteVisitor;
      ...
      //编辑
      orderEdit(inviteVisitor){
        this.$router.openActivity("inviting_visitors","slide-left",{uuid:inviteVisitor.uuid});
      }
    }
  }
</script>
<style lang="less" rel="stylesheet/less">
  .order-invite-container{
    width: 100%;
    height: 100%;
    .order-top{
      margin: auto;
