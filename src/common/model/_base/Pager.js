...
export default class Pager extends Base {
  ...
  httpCustomPage (url, params, successCallback, errorCallback) {
    ...
    if (this.history) {
      history.replaceState(history.state, '', Vue.store.state.route.path + '?' + $.param(params))
    }
  ...
  httpNextPage (url, params, fullCallback, successCallback, errorCallback, afterConcat) {
    ...
    this.httpGet(url, params, function (response) {
      ...
      tempPage.render(response.data.data);
      if(tempPage.data.length === 0) {
        fullCallback && fullCallback(response)
        return;
      }
      afterConcat && afterConcat(tempPage);
      that.page++;
      that.data = that.data.concat(tempPage.data);
      successCallback && successCallback(response)
    }, errorCallback)
  };
