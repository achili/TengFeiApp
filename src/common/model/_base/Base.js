...
export default class Base {
  ...
  httpPost(url, params, successCallback, errorCallback, opts = {}) {
    ...
    this.loading = true;
    if(params.version) {
      console.error('version字段被用于标识app版本，请勿使用');
    } else {
      params.version = Vue.store.state.version;
    }
    Vue.http.post(fullUrl, params, options).then(function (response) {
      that.loading = false;
      if (response.data.code === 201) {
        store.state.plus.nativeUI.toast(response.data.msg,{duration:"short"});
      }
